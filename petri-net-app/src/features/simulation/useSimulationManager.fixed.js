...
const useSimulationManager = (elements, setElements) => {
  ...
  const stepSimulation = useCallback(async () => {
    // Get the latest enabled transitions to ensure we have current state
    const enabled = await getEnabledTransitions();
    
    // If no transitions are enabled, stop the continuous simulation
    if (!enabled || enabled.length === 0) {
      stopAllSimulations();
      return;
    }
    
    // Otherwise, fire a random transition
  ...
  const startContinuousSimulation = useCallback(async () => {
    ...
    setEnabledTransitionIds(enabled.map(t => t.id));
